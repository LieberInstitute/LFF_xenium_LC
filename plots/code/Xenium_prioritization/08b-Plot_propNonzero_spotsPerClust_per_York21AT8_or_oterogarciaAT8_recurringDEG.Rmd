---
title: "08b-Plot_propNonzero_spotsPerClust_per_York21AT8_or_oterogarciaAT8_recurringDEG"
...
```{r}
clprop <- fread("processed-data/Xenium_prioritization/08a-ProportionSpots_per_clus_w_Oterogarcia22_multiclust_or_York21_AT8DEGs.txt")
```
...
plts <- lapply(unique(clprop$gene),FUN=function(g){
    ...
    p <- ggplot(pltdat,aes(y=cluster,x=prop_nonzero))+
        geom_col()+
        ggtitle(paste0(g," (",pltdat$source,")"))+
        theme(title=element_text(size=8),plot.title.position="plot")
    return(p)
})

pdf("plots/Xenium_prioritization/08b-PropClusterSpots_xpr_oterogarcia_or_york_AT8neuron_DEGs.pdf",height=4*ceiling(length(plts)/2),width=14)
