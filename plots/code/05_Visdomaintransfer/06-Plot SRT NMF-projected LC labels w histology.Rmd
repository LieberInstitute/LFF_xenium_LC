...
nmfpred <- fread("processed-data/05_SRTlabelTransfer/06-LCsmoothings_LUT.txt")
nmfpred <- DataFrame(nmfpred[,.(rn,nmfpred)],row.names=nmfpred$rn)[colnames(spe),]
spe$label <- nmfpred$nmfpred

## only plot LC
spe$label[!(spe$label %in% c("LC"))] <- NA
rm(nmfpred)
...
pal <- c(pal,NA)
names(pal)[2] <- NA
plt <- vis_grid_clus(spe,clustervar = "label",return_plots = T,spatial=T,is_stitched = T,image_id = "lowres",alpha = 0.425,na_color = NA,colors = pal,point_size = 17.5)
