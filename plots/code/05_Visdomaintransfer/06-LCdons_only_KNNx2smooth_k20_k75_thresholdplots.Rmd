---
title: "06-LCdons_only_KNNx2smooth_k20_k75_thresholdplots.Rmd"
...
## start with 3/75 (4%) neighbs up to 45/75 (60%) in steps of 6/75 (.08) for k2 thresh, since this makes such a stupid amount of plots
```{r}
k2thresh <- seq((3/75),(45/75),by=(6/75))

setDTthreads(1,restore_after_fork = FALSE)
i<-1
for (i in c(1:length(dualscores))){
  d <- copy(dualscores[[i]])
  n <- names(dualscores)[i]
  k1 <- as.numeric(gsub(n,pattern="k1thresh_(.*)",replacement="\\1"))
  j <- 1
  for (j in c(1:length(k2thresh))){
    k2 <- k2thresh[j]
    d[,isLC:=ifelse(lcpred.smooth1.k20>k1&lcpred.smooth2.k75>k2,"LC","Other")]   
    scrah <- as.data.frame(d,row.names=d$rn)[colnames(spe),]
    spe$newcol <- scrah$isLC
    colnames(colData(spe))[ncol(colData(spe))] <- paste0(n,"_thresh2_",k2)
    rm(scrah,k2)
  }
  rm(j,n,d,k1)
}
...
pltconds <- grep(colnames(colData(spe)),pattern="k1thresh",value=T)

mclapply(pltconds,mc.cores=4,function(n){
  plts <- lapply(unique(spe$brnum),function(s){
      p <- make_escheR(spe[,spe$brnum==s], y_reverse=FALSE)
      p <- p |> add_fill(n,size=0.7,point_size=0.7)
      p <- p+scale_fill_manual(
        name = paste0(n),
        values = pal,
        guide = guide_legend(override.aes = list(size = 3)))
      p <- p+ggtitle(s)
      p <- rasterize(p,layers = "Points",dev="cairo_png",dpi = 150)
      return(p)
  })
  
  png(paste0("plots/05_Visdomaintransfer/06-LCdonsonly_VisLabXfer_smoothed_KNN_",n,".png"),res=150,unit="in",height=36,width=22)
