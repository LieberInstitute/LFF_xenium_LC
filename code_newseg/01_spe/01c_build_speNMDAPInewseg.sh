...
## Run code
brnum=$(awk -v n="$SLURM_ARRAY_TASK_ID" 'BEGIN{FS="\t"} NR==n {print $1}' /dcs05/lieber/marmaypag/LFF_spatialLC_LIBD4140/LFF_xenium_LC/code/xenium_imageProcessing_new/inputs.txt | awk '{print $1}')
echo "$brnum"

export BRNUM=$brnum
Rscript 01c_build_speNMDAPInewseg.R
