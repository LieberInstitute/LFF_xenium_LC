...
# load SPE
```{r}
lcx <- readRDS("processed-data_newseg/01_spe/NMDAPI_raw_combined_spe.RDS")
# fix brnums and drop 1691
lcx$brnum <- gsub(lcx$brnum,pattern=" $",replacement="")
lcx$brnum <- gsub(lcx$brnum,pattern="-",replacement="")

## Br1691 right hemi excluded as before; 1691L to be retained for now. 
...
```{r}
png("plots_newseg/02_prefiltQC/01a-NMDAPI_cellArea_vs_ncounts.png",height=1000,width=1250)
...
png("plots_newseg/02_prefiltQC/01c-NuniqueGenes_vs_totCounts.png",height=500,width=600)
...
```{r}
png("plots_newseg/02_prefiltQC/01d-nucleusArea_vs_cellArea.png",height=1000,width=1250)
...
png("plots_newseg/02_prefiltQC/01e-cellArea_by_cell_in_space.png",width=8250,height=8250)
...
png("plots_newseg/02_prefiltQC/01f-nuc_frac_fulldata_and_bysample.png",height=1000,width=2250)
...
# 212 (13 new compared to above)
sum(lcx$nucleus_area>0.95*lcx$cell_area)
# 258 (59 new compared to nucleus-cell matched areas)
```
...
png("plots_newseg/02_prefiltQC/01g-nucleusArea_vs_propNuc.png",height=1000,width=1250)
plt
dev.off()
```
...
plts[[34]] <- rasterize(plotColData(lcx,x="nuc_frac",y="cell_area"))+ggtitle("Full Dataset Cell Area vs. Prop Nuc Area")

plts <- plts[c(34,1:33)]

png("plots_newseg/02_prefiltQC/01h-cellArea_vs_propNuc.png",height=4125,width=4125)
do.call(grid.arrange,c(plts,ncol=4))
...
for (i in c(1:length(plts))){
    pdf(paste0("plots_newseg/02_prefiltQC/01i-",names(plts)[i],".pdf"),height=48,width=24)
...
pdf("plots_newseg/02_prefiltQC/01j-subsets_any_neg_detected.pdf",height=48,width=24)
...
## lastly, per Maddy's own workup of the NM-DAPI joint segmentation, this produced a spurious number of false positive nuclei:
DAPI(nucleus_area<4μm2) = 86 --> NMDAPI(nucleus_area<4μ2) = 26822 (with the new segmentation)

we'll remove (remaining) nuclei smaller than 4µm2 here as with the original NMDAPI.
```{r}
lcx$falsenuc <- lcx$nucleus_area<4
sum(lcx$falsenuc) #26822
```
...
saveRDS(lcx,"processed-data_newseg/02_QC/01a-spe_raw_controlOutliers_tinynuc_marked.RDS")
...
## get a table of how many outlier cells per outlier type per sample
...
fwrite(ans,"processed-data_newseg/02_QC/01b-number_and_prop_outlierCells_bytype_persamp.txt",sep='\t',quote=F,row.names=F,col.names=T)
rm(cz,ans,ncell)
```
...
lapply(outliervars,FUN=function(v){
    ...
    png(paste0("plots_newseg/02_prefiltQC/01k-",v,"_spatial.png"),height=8250,width=8250)
...
png("plots_newseg/02_prefiltQC/01l-outlier_cellAreas.png",height=2500,width=1750)
...
sum(colSums(counts(lcx))==0) # 5194 
lcx <- lcx[,colSums(counts(lcx))>0] # 

### we can get the cell ids to keep from the data.table of coldata (where the cell ids are rownames) to drop nuclei we know to be nM segmentations by their large area that lack LC markers
...
# (xeniumranger DAPI nuclei do not exceed area of 400, thus we know those with area>400 are from NM); if we find any such bodies devoid of LC markers, we can presume them to be erroneous NM segmentations (i.e., erroneous 'nuclei' around which a cell boundary was nonetheless drawn, which are cells we should be rid of).  
dropbiggies <- tmpfilttab[DBH==0&TH==0&SLC6A2==0&nucleus_area>400,rn] 
# only 45 cells meet this criterion
...
dim(lcx)
# 541 
# 530729 (530774 before removing the 45 NM non-NE cells)
...
# finally, get (and keep separately for a smol exploration--the cells with "nuclear" (i.e. NM+DAPI) area <4µm^2. If these are enriched for say macrophage/microglia markers and not for blood/vascular markers, could be engulfed NM which would be quite interesting in its own right.)
smolnucs <- lcx[,lcx$falsenuc]
dim(smolnucs) # 541 / 21251

lcx <- lcx[,lcx$falsenuc==FALSE]
# dim (lcx)
# 541 491782
```
...
```{r}
saveRDS(lcx,"processed-data_newseg/02_QC/01b-spe_controlOutliers_and_tinynuc_and_giantLCmarkerlessNuclei_filtered.RDS")
saveRDS(smolnucs,"processed-data_newseg/02_QC/01c-spe_NMDAPI_nuclei_under4umSq_only.RDS")
```
