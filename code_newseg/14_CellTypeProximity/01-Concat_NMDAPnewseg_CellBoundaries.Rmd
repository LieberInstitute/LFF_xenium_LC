---
title: "01-Concat_NMDAPnewseg_CellBoundaries"
...
## made a copy of the cell boundary output files buried in processed-data/xenium_imageprocessing_new/ so they're all in one directory for this part
```{r}
spe <- readRDS("processed-data_newseg/02_QC/02b-spe_filtered_genetargonly_log-and-nonlog-norms.RDS")

cbs <- list.files("processed-data_newseg/14_CellTypeProximity/00-NMDAPI_cell_boundaries",full.names=T)

cbdts <- lapply(cbs,function(x){
  brnum <- gsub(x,pattern="^.*/(Br.*)_cell_boundaries.csv.gz$",replacement="\\1")
  brnum <- gsub(brnum,pattern="-L",replacement="L")
  dt <- fread(x)
  dt[,cell_id:=paste0(brnum,"_",cell_id)]
  dt[,label_id:=NULL]
  dt <- dt[cell_id %in% colnames(spe),]
  return(dt)

})
allcbd <- rbindlist(cbdts)
allcbd <- allcbd[cell_id %in% colnames(spe)]
stopifnot(length(unique(allcbd$cell_id))==ncol(spe))


fwrite(allcbd,"processed-data_newseg/14_CellTypeProximity/retainedCellBoundaries.txt.gz",sep='\t',quote=F,row.names=F)
```
...
loaded via a namespace (and not attached):
 ...
 [5] vctrs_0.7.1         tools_4.5.2         parallel_4.5.2      tibble_3.3.1       
 [9] pkgconfig_2.0.3     R.oo_1.27.1         Matrix_1.7-4        RColorBrewer_1.1-3 
[13] S7_0.2.1            lifecycle_1.0.5     compiler_4.5.2      farver_2.1.2       
[17] codetools_0.2-20    htmltools_0.5.9     yaml_2.3.12         pillar_1.11.1      
[21] R.utils_2.13.0      rsconnect_1.7.0     DelayedArray_0.36.1 sessioninfo_1.2.3  
...
[41] rmarkdown_2.30      XVector_0.50.0      otel_0.2.0          R.methodsS3_1.8.2  
