...
## Br1691 right hemi excluded at SPE construction; 1691L to be retained for now. 
...
```{r}
png("plots/01_prefiltQC/01a-NMDAPI_cellArea_vs_ncounts.png",height=1000,width=1250)
...
png("plots/01_prefiltQC/01c-NuniqueGenes_vs_totCounts.png",height=500,width=600)
...
```{r}
png("plots/01_prefiltQC/01d-nucleusArea_vs_cellArea.png",height=1000,width=1250)
...
png("plots/01_prefiltQC/01e-cellArea_by_cell_in_space.png",width=8250,height=8250)
...
png("plots/01_prefiltQC/01f-nuc_frac_fulldata_and_bysample.png",height=1000,width=2250)
...
# 2132 (1 new compared to above)
sum(lcx$nucleus_area>0.95*lcx$cell_area)
# 2139 (8 new compared to nucleus-cell matched areas)
```
...
png("plots/01_prefiltQC/01g-nucleusArea_vs_propNuc.png",height=1000,width=1250)
plt
dev.off()
```
...
png("plots/01_prefiltQC/01h-cellArea_vs_propNuc.png",height=4125,width=4125)
do.call(grid.arrange,c(plts,ncol=4))
...
for (i in c(1:length(plts))){
    pdf(paste0("plots/01_prefiltQC/01i-",names(plts)[i],".pdf"),height=48,width=24)
...
pdf("plots/01_prefiltQC/01j-subsets_any_neg_detected.pdf",height=48,width=24)
...
DAPI(nucleus_area<4μm2) = 86 --> NMDAPI(nucleus_area<4μ2) = 40282 
DAPI(nucleus_area<5μm2) = 3158 --> NMDAPI(nucleus_area<5μ2) = 43315 

we'll remove (remaining) nuclei smaller than 4µm2 here. 
```{r}
lcx$falsenuc <- lcx$nucleus_area<4
sum(lcx$falsenuc) #40418
```
...
saveRDS(lcx,"processed-data/02_QC/01a-spe_raw_controlOutliers_tinynuc_marked.RDS")
...
fwrite(ans,"processed-data/02_QC/01b-number_and_prop_outlierCells_bytype_persamp.txt",sep='\t',quote=F,row.names=F,col.names=T)
rm(cz,ans,ncell)
```
...
lapply(outliervars,FUN=function(v){
    ...
    png(paste0("plots/01_prefiltQC/01k-",v,"_spatial.png"),height=8250,width=8250)
...
png("plots/01_prefiltQC/01l-outlier_cellAreas.png",height=2500,width=1750)
...
# only 48 other cells meet this criterion
...
dim(lcx)
# 541 
# 538842 (before removing 48 big fellas 538890)
...
```{r}
saveRDS(lcx,"processed-data/02_QC/01b-spe_controlOutliers_and_tinynuc_and_giantLCmarkerlessNuclei_filtered.RDS")
saveRDS(smolnucs,"processed-data/02_QC/01c-spe_NMDAPI_nuclei_under4umSq_only.RDS")
```
