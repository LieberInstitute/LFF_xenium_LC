...
crossval <- RcppML::crossValidate(logcounts(lcv),k=seq(61,89,1),tol=1e-5,threads=15,alpha=0,verbose=T)


saveRDS(crossval, "processed-data/00_prelim_procdat_BJM/05_LabelTransfer/01b-Crossval_VisNMF_61to89.RDS")

library(ggplot2)
png("plots/00_prelimplotsBJM/05_Visdomaintransfer/01b-VisNMF_crossval_k61to89.png", height=4,width=8, unit="in",res=300)
ggplot(crossval,aes(x=k,y=value))+geom_point()+scale_x_continuous(breaks=seq(60,90,5))
dev.off()
