...
lcx <- readRDS("processed-data/00_prelim_procdat_BJM/04_Banksy_clusterings/01-staggeredCoord_spe_genetargOnly_NONlogNorm.RDS")

# get SLURM job #, i.e. row from the parameter table to use
job <- as.numeric(Sys.getenv("SLURM_ARRAY_TASK_ID"))

# get the parameter table
parms <- fread("processed-data/00_prelim_procdat_BJM/04_Banksy_clusterings/01-BanksyParamSets.txt")
...
fwrite(outtab,
       paste0("processed-data/00_prelim_procdat_BJM/04_Banksy_clusterings/01-banksy_clustruns_outs/",
              names(outtab)[2],
              ".txt"),
       sep='\t',
       quote=F,
