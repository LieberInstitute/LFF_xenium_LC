---
title: "06a-LCsmoothing_knnstep1_k20"
...
## no samples to drop from the xenium data--the only difference is in the visium data, where we dropped c(5517,5276,5712) before doing any processing in this case (instead of processing+clustering, then dropping, as in the main).

labxf <- fread("processed-data/99_UsingVisSensitivityDomains/01_labelTransfer/05c-LCdonsonly_labelxfer_concattab.txt")

labxf <- DataFrame(labxf,row.names=labxf$rn)[colnames(spe),]
spe$nmfpred <- labxf$nmf_preds
...
saveRDS(xyspeout,"processed-data/99_UsingVisSensitivityDomains/01_labelTransfer/06a-LCknnsmoothing_step1k20.RDS")
```
