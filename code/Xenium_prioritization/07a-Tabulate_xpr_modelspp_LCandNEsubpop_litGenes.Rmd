---
title: "07a-Tabulate_xpr_modelspp_LCandNEsubpop_litGenes"
...
fetch table of subpopulation, development markers from literature
```{r}
poplc <- fread("processed-data/Xenium_prioritization/07a-Lit_NE_subpopAndDevOrigin_mks.txt")

## HOXB1 is not in the SPE at all (nor is its ensembl ID, ENSG00000120094)
poplc <- poplc[gene_name!="HOXB1"]

lc3 <- lc3[unique(poplc$gene_name),]
```
...
# save
fwrite(anstab,"processed-data/Xenium_prioritization/07a-ProportionSpots_per_clus_w_Literature_adultOrDevelLC-subpop-mks.txt",sep='\t',quote=F,row.names=F)
```
