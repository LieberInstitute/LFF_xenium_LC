---
title: "TEST-LMM_NMmetrics_to_spotlogcounts"
...
nmcors <- lapply(c("Intensity_NM","Prop_NM"),function(nm){
    results <- mclapply(c(1:nrow(lcnm.lct)),mc.cores=10,FUN=function(i){
        ...
        # fit lmms
        lmmnm <- lmer(NMvar ~ val + Age + Sex + e4copies + (1|brnum) + (1|brnum:sample_id),data=xpr)
        
        ## to perform LRT test, we shouldn't use REML, so run a second non REML
        lmmnm.compare <- lmer(NMvar ~ val + Age + Sex + e4copies + (1|brnum) + (1|brnum:sample_id),data=xpr,REML = FALSE)
        lmmno <- lmer(NMvar ~ Age + Sex + e4copies + (1|brnum) + (1|brnum:sample_id),data=xpr,REML=FALSE)
        ...
        ## for some reason Sex comes out as Sex here but "SexM" for the fixef. so to make sure we keep the sex coefficients, change SexM to Sex here
        fixfx[rn=="SexM",rn:="Sex"]
...
saveRDS(nmcors2,"processed-data/Xenium_prioritization/TEST-LCNMspots_NMmetric_LMMed_by_geneLogcounts.RDS")
```

## despite the flood of singular fit warnings with the gene ~ NM setup / the absence of them with this NM ~ gene setup, the top hits have similar ranges of p values and significant LRT tests to boot. *shrug*
...
[13] scuttle_1.16.0          rmarkdown_2.29          UCSC.utils_1.2.0       
[16] nloptr_2.1.1            ggbeeswarm_0.7.2        purrr_1.0.2            
[19] xfun_0.50               zlibbioc_1.52.0         beachmat_2.22.0        
[22] jsonlite_1.8.9          DelayedArray_0.32.0     BiocParallel_1.40.0    
[25] irlba_2.3.5.1           R6_2.5.1                boot_1.3-31            
[28] Rcpp_1.0.14             knitr_1.49              zoo_1.8-12             
[31] splines_4.4.2           tidyselect_1.2.1        rstudioapi_0.17.1      
[34] abind_1.4-8             yaml_2.3.10             viridis_0.6.5          
[37] codetools_0.2-20        lattice_0.22-6          tibble_3.2.1           
[40] evaluate_1.0.3          pillar_1.10.1           rsconnect_1.3.3        
